...
func (t *Table) ForMap(fn data.MapIterator, opts ...data.TableOpt) {
	...
	// Create the map
	m := make(map[string]interface{}, len(t.header.order))
...
func (t *Table) doReadRow(order []int, row []string) error {
	...
	for i, cell := range row {
		if order == nil {
			r.set(i, cell, t.valueForString(cell))
		} else {
			r.set(order[i], cell, t.valueForString(cell))
		}
	}
...
func (t *Table) valuesForRow(row *row) []interface{} {
	result := make([]interface{}, t.w)
	for i := range result {
		result[i] = row.get(i, t.optFlag)
	}
	return result
}
